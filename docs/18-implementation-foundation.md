...
3. Start PostgreSQL/PostGIS with `docker compose up -d`.
4. Run `npm install`.
5. Run `npm run db:migrate`.
6. Run `npm run db:seed`.
7. Optionally run `SEED_DEMO_CONTENT=true npm run db:seed` to add a clearly fictional demo business.
8. Run `npm run dev`.
...
`npm run compatibility` imports and constructs the selected integration boundaries:
...
- Registration UI is not included yet.
...
- A committed npm lockfile remains a follow-up repository task; direct dependencies are pinned and CI currently installs from the manifest.
