...
import {
  loadCategorySeed,
  loadPlaceSeed,
} from "../src/modules/reference-data/loader";
    ...
    await db
      ...
      .values({
        ...
        coordinate: place.coordinate
          ? point(place.coordinate.longitude, place.coordinate.latitude)
          : null,
        source: place.source,
    ...
    await db
      ...
      .values({
        ...
        parentId: category.parentSlug
          ? idsBySlug.get(category.parentSlug)
          : null,
        slug: category.slug,
...
async function seedDemoBusiness() {
  ...
  const [{ businesses, businessServices }] = await Promise.all([
    import("../src/lib/database/schema/businesses"),
  ]);

  const [category] = await db
    .select({ id: categories.id })
    .from(categories)
    .where(eq(categories.slug, "plumbers-heating"))
    .limit(1);
  const [place] = await db
    .select({ id: places.id })
    .from(places)
    .where(eq(places.slug, "treorchy"))
    .limit(1);

  if (!category || !place) {
    throw new Error("Reference data must exist before demo content is seeded.");
  }
  ...
  await db
    ...
    .values({
      ...
      summary:
        "A fictional profile used to prove the generated business website journey.",
      description:
        "This is demo content, not a real trader. It demonstrates how one structured business record powers both directory discovery and a generated public website.",
      status: "published",
  ...
  await db
    ...
    .where(eq(businessServices.businessId, demoId));
